...
// });
// export const UploadQr = async (image: File): Promise<string | NextResponse> => {
//   if (!image) {
...
export const UploadQr = async (image: File): Promise<string | NextResponse> => {
  ...
  try {
      ...
      const uploadStream = cloudinary.uploader.upload_stream(
        ...
        (error: any, result: any) => {
          if (error) {
            ...
            resolve(result.public_id); // Return public_id instead of URL
          } else {
            reject(new Error("Unexpected Cloudinary response."));
          }
        }
    ...
    return publicId; // Return the public_id
  } catch (error) {
    console.error("Error uploading image:", error);
    return NextResponse.json(
      { error: "Failed to upload image. Please try again later." },
